...
* 窗口消息：一般与窗口的内部运作有关，如创建窗口，绘制窗口和销毁窗口等。消息是从系统发送到窗口，或从窗口发送到窗口。
* 命令消息：一般与处理用户请求相关，当用户点击一个菜单或工具栏时，命令消息产生，并被发送到能处理该请求的类对象（如，装载文件，编辑文本，保存选项等）
* 控件通知：通常在某些重要事件发生时，有控件窗口发送到父窗口，如打开一个组合框。控件通知为父窗口进一步控制子窗口提供了机会。
...
### 画图：
 CPaintDC类响应WM_PAINT消息，允许你在窗口客户区画图。
